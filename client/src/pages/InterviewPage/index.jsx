...
import InterviewActionsLog from '../../components/InterviewActionsLog';
...
function InterviewPage() {
  ...
  // Track whether we've already requested microphone permission to avoid repeated prompts
...
// Prompt for microphone permission when transitioning to listening state (so user gets the permission dialog before recording)
useEffect(() => {
  if (interviewerState === STATE_LISTENING && !micPermissionRequested && typeof navigator !== 'undefined' && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    (async () => {
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        // immediately stop tracks - we just wanted to trigger the permission prompt
        stream.getTracks().forEach((t) => t.stop());
      } catch (err) {
        // user denied or there was an error - log but don't block
        console.warn('Microphone permission prompt failed or was denied:', err?.message || err);
      } finally {
        setMicPermissionRequested(true);
      }
    })();
  }
}, [interviewerState, micPermissionRequested]);
...
const handleEndInterview = async () => {
  ...
  return (
    <div className="interview-layout">
      ...
      {interviewData && <InterviewActionsLog interview={interviewData} />}
