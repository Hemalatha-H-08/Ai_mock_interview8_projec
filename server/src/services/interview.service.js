...
const createInterviewRecord = async (data) => {
  ...
  demoStore.interviews.set(String(interview._id), interview);
  return interview;
};

const getInterviewRecord = async (interviewId, userId) => {
  if (isDatabaseConnected()) {
    return Interview.findOne({ _id: interviewId, userId });
  }

  const interview = demoStore.interviews.get(String(interviewId));
...
const saveInterviewRecord = async (interview) => {
  if (isDatabaseConnected()) {
...
export const startInterview = async (userId, role, resumeText, candidateName, totalQuestions = 5) => {
  ...
  await saveInterviewRecord(interview);
...
export const submitAnswer = async (interviewId, userId, answerText) => {
  const interview = await getInterviewRecord(interviewId, userId);
