...
const startServer = async () => {
  try {
    // Step 1: Try to connect to MongoDB. If it fails, continue in demo mode.
    const dbAvailable = await connectDB();
    ...
    app.listen(PORT, () => {
      ...
      if (!dbAvailable) {
        console.error(' Mode: demo mode (in-memory data fallback enabled)\n');
      } else {
        console.error('');
      }
    });
